package com.turbouml.dao;

import com.turbouml.dto.enums.Access;
import com.turbouml.dto.models.*;
...
public class ClassDiagramDao
{
    ...
    @Autowired
    public ClassDiagramDao(DSLContext create) {
        this.create = create;
    }

    private List<UMLClassDiagram> getResult(ResultSet rs)
        throws SQLException
    {
        List<UMLClassDiagram> diagrams = new ArrayList<>();

        while(rs.next()) {
            var obj = new UMLClassDiagram();
    ...
    private List<UMLField> sqlArrayToFieldsList(Array sqlArray)
    {
        try {
            List<UMLField> classFields = new ArrayList<>();
            if(sqlArray == null) {
                return classFields;
            }
            String[][] rows = (String[][]) sqlArray.getArray();
            for (String[] columns : rows) {
                var obj = new UMLField();
    ...
    private List<UMLMethod> sqlArrayToMethodsList(Array sqlArray)
    {
        try {
            List<UMLMethod> classMethods = new ArrayList<>();
            if(sqlArray == null) {
                return classMethods;
            }
            String[][] rows = (String[][]) sqlArray.getArray();
            for (String[] columns : rows) {
                var obj = new UMLMethod();
    ...
    public List<UMLClassDiagram> findByProjectId(String projectId) throws SQLException
    {
